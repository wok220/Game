...
public class WaveManager {
    ...
    public void initCampaign(int levelIndex, GameManager gameManager) {
        ...
        // 创建简单波次：10只普通僵尸，每只间隔5秒
        currentWaveSpawns = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            int row = i % Constants.GRID_ROWS;
            int delay = i * 5000;  // 每5秒一只
            currentWaveSpawns.add(new GameConfig.ZombieSpawn("NormalZombie", row, delay));
        }
    ...
    public void initEndless(GameManager gameManager) {
        ...
        // 生成第一波（只生成普通僵尸）
        generateSimpleEndlessWave();
    }

    /**
     * 生成简单的无限模式波次（只生成普通僵尸）
     */
    private void generateSimpleEndlessWave() {
        endlessWaveNumber++;

        // 每波僵尸数量逐渐增加（5只起，每波+2）
        int zombieCount = 5 + (endlessWaveNumber - 1) * 2;
        if (zombieCount > 30) {
            zombieCount = 30;  // 最多30只
        }
